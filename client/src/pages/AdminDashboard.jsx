...
export default function AdminDashboard() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/shops/admin/${currentUser.uid}`);
      ...
      if (currentShopId) {
        const qRes = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/queue/status/${currentShopId}`);
        if (qRes.ok) {
  ...
  const handleComplete = async () => {
    try {
      if (!shopId) return;
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/queue/call-next`, {
        method: 'POST',
